...
class TwoWayHyperFlowCutterRefiner final : public IRefiner,
 ...
 private:
  ...
  bool refineImpl(std::vector<HypernodeID>&, const std::array<HypernodeWeight, 2>&,
    ...
    hfc.cs.setMaxBlockWeight(0, std::max(_hg.partWeight(0), _context.partition.max_part_weights[b0]));
    hfc.cs.setMaxBlockWeight(1, std::max(_hg.partWeight(1), _context.partition.max_part_weights[b1]));
